package net.moddingplayground.frame.impl.client.render.entity;
...
public class FrameBoatEntityRenderer<T extends FrameBoatEntity> extends EntityRenderer<T> {
    public static final Function<Identifier, Identifier> TEXTURES = Util.memoize(FrameBoatEntityRenderer::texture);
    public static final BiFunction<EntityRendererFactory.Context, Identifier, BoatEntityModel> MODELS = Util.memoize(FrameBoatEntityRenderer::modelLayer);

    private final EntityRendererFactory.Context context;

    public FrameBoatEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
    ...
    public void render(T entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertices, int light) {
        if (!entity.hasValidWoodType()) return;
    ...
    public static BoatEntityModel modelLayer(EntityRendererFactory.Context ctx, Identifier id) {
        WoodBlockSet set = FrameRegistry.WOOD.get(id);
        EntityModelLayer layer = FrameEntityModelLayers.BOAT_LAYERS.get(set);
